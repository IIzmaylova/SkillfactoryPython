...
def ConvertNum(element):
    try:
        if float(element):                      # Элемент является числом?
...
def get_NumSequense():
    ...
    try:
        ...
        elif not all(array):                    # Вызываем исключение, если в последовательности есть нечисловые значения
...
def get_UserNum():
    ...
    try:
        ...
        if all(user_num_):                          # Если получено число, присваиваем переменной первый элемент массива
